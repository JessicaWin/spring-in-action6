...
public class FluxTest {
  ...
  public void createAFlux_range() {
    ...
    StepVerifier.create(intervalFlux).expectNext(1).expectNext(2).expectNext(3).expectNext(4)
  ...
  public void createAFlux_interval() {
    ...
    StepVerifier.create(intervalFlux).expectNext(0L).expectNext(1L).expectNext(2L).expectNext(3L)
  ...
  public void zipFluxesToObject() {
    ...
    StepVerifier.create(zippedFlux).expectNext("Garfield eats Lasagna")
        .expectNext("Kojak eats Lollipops").expectNext("Barbossa eats Apples").verifyComplete();
  }
  ...
  public void map() {
    ...
    StepVerifier.create(playerFlux).expectNext(new Player("Michael", "Jordan"))
        .expectNext(new Player("Scottie", "Pippen")).expectNext(new Player("Steve", "Kerr"))
        .verifyComplete();
  }
  ...
  public void bufferAndFlatMap() throws Exception {
    Flux.just("apple", "orange", "banana", "kiwi", "strawberry").buffer(3).flatMap(x -> Flux
        .fromIterable(x).map(y -> y.toUpperCase()).subscribeOn(Schedulers.parallel()).log())
        .subscribe();
  }
